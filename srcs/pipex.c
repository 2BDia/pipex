...
/*   Updated: 2021/08/16 14:01:49 by rvan-aud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	child(char ***cmd, char **env, int *pipefd, char **path, int n)
{
	int		fd;

	fd = open(*cmd[n - 2], O_RDONLY);
	if (fd == -1)
...
void	pipex(char ***cmd, char **env, int *pipefd, int n)
{
	...
	if (pid == 0)
		child(cmd, env, pipefd, path, n);
	wait(0);
	fd = open(*cmd[n - 1], O_RDWR | O_CREAT | O_TRUNC, 0666);
	if (fd == -1)
